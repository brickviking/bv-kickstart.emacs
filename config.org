...
If you are interested in these other package managers, check out their respective github repositories or search for more package managers.
...
#+begin_src emacs-lisp
    (use-package evil
      ...
      (setq evil-want-keybinding nil) ;; Disable loading evil bindings for other modes (It's not consistent and not good)
...
Once you've used it for a bit, you'll understand why people love it. Even reading about it can be inspiring! For example, this document is effectively the source code and descriptions bound into the one document, much like the literate programming ideas that Donald Knuth made famous.
...
By default, the shell is set to 'fish' but you could change it to 'bash' or 'zsh'.
...
** Nerd Icons
Includes more icons; don't forget to use nerd-icons-install-fonts.
For doom modeline:
#+begin_src emacs-lisp
